...
test('parseRepairError extracts paths from a partial-failure message', () => {
  const error = parseRepairError(
    'bpp_data_reset_partial_failure:C:/Games/The Bazaar/BazaarPlusPlus/bazaarplusplus.db\u{1f}C:/Games/The Bazaar/BazaarPlusPlus/Identity/auth.v1.json'
  );
  ...
  expect(error).toEqual({
    ...
    failedPaths: [
      ...
      'C:/Games/The Bazaar/BazaarPlusPlus/Identity/auth.v1.json'
    ]
  });
});
