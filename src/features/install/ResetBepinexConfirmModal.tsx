import { AlertTriangle, FolderX, Loader2, ShieldCheck, X } from 'lucide-react';
...
export function ResetBepinexConfirmModal({
  ...
  return (
    <Dialog onClose={onClose} labelledBy="reset-bepinex-modal-title">
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <label className="flex items-start gap-3 p-3 border border-[rgba(190,80,80,0.22)] rounded-[4px] bg-[rgba(160,50,50,0.06)] group">
            ...
            <span className="text-[13px] leading-relaxed text-[rgba(245,220,220,0.82)]">
              {t('resetBepinexConfirmAcknowledge')}
            </span>
          </label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="button"
              disabled={!acknowledged || page.action === 'resetBepinex'}
              onClick={onConfirm}
            ...
            >
              {page.action === 'resetBepinex' ? (
                <span className="inline-flex items-center gap-2">
                  <Loader2 size={14} className="animate-spin" />
                  {t('resetBepinexConfirmAction')}
                </span>
              ) : (
                t('resetBepinexConfirmAction')
              )}
